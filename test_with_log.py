#!/usr/bin/env python3
"""
Test script that writes results to a log file
"""
...
def test_conversion():
    with open('test_results.log', 'w') as log_file:
        log_print("=== MIDI Round-trip Conversion Test ===", log_file)
        ...
        try:
            ...
            test_midi.save(test_file)
            log_print(f"Created test MIDI file: {test_file}", log_file)
            
            # Load original MIDI
            original_midi = mido.MidiFile(test_file)
            log_print(f"Original MIDI: {len(original_midi.tracks)} tracks, {original_midi.ticks_per_beat} ticks_per_beat", log_file)
            
            # Count original messages
            orig_msg_count = sum(len(track) for track in original_midi.tracks)
            log_print(f"Original message count: {orig_msg_count}", log_file)
            
            # Generate XML using the same logic as the main application
            xml_content = generate_xml_from_midi(original_midi)
            log_print(f"Generated XML length: {len(xml_content)} characters", log_file)
            
            # Parse the XML to check structure
            root = ET.fromstring(xml_content)
            xml_tracks = root.findall('Track')
            log_print(f"XML contains {len(xml_tracks)} tracks", log_file)
            
            xml_msg_count = sum(len(track.findall('Message')) for track in xml_tracks)
            log_print(f"XML message count: {xml_msg_count}", log_file)
            ...
            log_print(f"Saved reconstructed MIDI: {output_file}", log_file)
            
            # Compare the results
            log_print(f"Reconstructed MIDI: {len(reconstructed_midi.tracks)} tracks, {reconstructed_midi.ticks_per_beat} ticks_per_beat", log_file)
            
            recon_msg_count = sum(len(track) for track in reconstructed_midi.tracks)
            log_print(f"Reconstructed message count: {recon_msg_count}", log_file)
            ...
            log_print(f"\nComparison Results:", log_file)
            log_print(f"Tracks match: {tracks_match} ({len(original_midi.tracks)} vs {len(reconstructed_midi.tracks)})", log_file)
            log_print(f"Ticks per beat match: {ticks_match} ({original_midi.ticks_per_beat} vs {reconstructed_midi.ticks_per_beat})", log_file)
            log_print(f"Message count match: {messages_match} ({orig_msg_count} vs {recon_msg_count})", log_file)
            
            if tracks_match and ticks_match and messages_match:
                log_print("\n✅ Round-trip conversion appears to be working correctly!", log_file)
            else:
                log_print("\n❌ Round-trip conversion has discrepancies.", log_file)
                
            # Test detailed message comparison
            log_print("\nDetailed message comparison:", log_file)
            ...
            for i, (orig_msg, recon_msg) in enumerate(zip(orig_track, recon_track)):
                log_print(f"Message {i}:", log_file)
                log_print(f"  Original: {orig_msg}", log_file)
                log_print(f"  Reconstructed: {recon_msg}", log_file)
                
                if orig_msg.dict() != recon_msg.dict():
                    log_print(f"  ❌ Messages differ!", log_file)
                else:
                    log_print(f"  ✅ Messages match", log_file)
                
        except Exception as e:
            log_print(f"Test failed with error: {e}", log_file)
            import traceback
